...
const normalizeRestaurant = (r) => {
  ...
  return {
    ...
    deliveryFee:
      r.deliveryFee ?? r.delivery_fee ?? r.merchant?.deliveryFee ?? r.merchant?.delivery_fee ?? 0,
    raw: r,
  };
};
...
const mergeCarts = (base = { items: [], total: 0 }, added = { items: [], total: 0 }) => {
  ...
  for (const newIt of added.items || []) {
    ...
    if (found) {
      ...
      items.push({
        ...newIt,
        id: newIt.id ?? `${Date.now()}_${Math.random().toString(36).slice(2, 9)}`,
      });
    }
  }
  ...
  switch (action.type) {
    ...
    case 'ADD_ITEM': {
      ...
      if (items.length > 0 && items[0].restaurant?.id !== restaurant.id) {
        ...
        return {
          ...
          toastInfo: 'Món từ nhà hàng khác → giỏ cũ đã bị xóa, chỉ giữ món mới.', // <--- đánh dấu
        };
      }
    ...
    case 'REMOVE_ITEM':
      return {
        ...
        total: recalcTotal((safeState.items || []).filter((it) => it.id !== action.payload)),
      };
    case 'UPDATE_QUANTITY': {
      ...
      if (quantity <= 0)
        return {
          ...
          total: recalcTotal((safeState.items || []).filter((it) => it.id !== id)),
        };
    ...
    if (userId !== 'guest') {
      ...
      if (guestCart.items && guestCart.items.length > 0) {
        ...
        if (!userRestaurantId || userRestaurantId === guestRestaurantId) {
          ...
          if (merged.items.length > 0 && userCart.items.length === 0) showToast = true;
        } else {
