...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      // SỬA LỖI: Lưu cả user và token vào localStorage
  ...
  const register = async (data) => {
    dispatch({ type: 'SET_LOADING', payload: true });
    try {
      // SỬA LỖI: Thêm http:// vào URL gọi API
      const response = await fetch('http://192.168.100.124:3000/api/registerCustomer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      // SỬA LỖI: Lưu cả user và token sau khi đăng ký
  ...
  // 🔹 Đăng xuất: Xóa sạch dữ liệu cũ và giỏ hàng (nếu có)
  const logout = () => {
    try {
      const previousUserId = state.user?.id || state.user?._id;

      // Xóa thông tin lưu trữ của phiên làm việc
      localStorage.removeItem('user');
      localStorage.removeItem('token');

      // Xóa giỏ hàng của user cũ ra khỏi bộ nhớ trình duyệt
      if (previousUserId) {
